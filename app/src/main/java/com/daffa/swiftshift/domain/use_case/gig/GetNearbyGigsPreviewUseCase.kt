...
class GetNearbyGigsPreviewUseCase @Inject constructor(
    private val repository: IGigRepository,
) {
    suspend operator fun invoke(latLng: LatLng): Flow<Resource<List<Gig>>> {
        return repository.getNearbyGigsPreview(latLng)
    }
}